...
let nameUser = localStorage.getItem('loginName')
if (nameUser) {
    document.getElementById('welcome').innerHTML = 'Welcome ' + nameUser
}
function signUp() {
    ...
    if(validationEmailLogin() == true && validationPassLogin() == true){   
        for (let i = 0; i < regList.length; i++) {
            ...
            if (isEmailAndPassExist(em,pas)) {
                ...
                localStorage.setItem('loginName', regList[isEmailAndPassExist(em,pas)].name)
            } else {
